...
export default function Home() {
  ...
  // 우주 배경일 때만 흰색, 나머지는 검은색
  const titleColor = isSpaceBackground ? 'text-white' : 'text-gray-900';
  const subtitleColor = isSpaceBackground ? 'text-gray-200' : 'text-gray-500';
  
  // 자연 배경일 때 소제목에 반투명 배경 추가
  const subtitleClasses = isNatureBackground 
    ? 'text-gray-900 bg-white/40 backdrop-blur-sm px-4 py-2 rounded-lg shadow-sm' 
    : subtitleColor;
  ...
  return (
    <>
      ...
      <div className="flex">
        {/* 중앙 메인 영역 */}
        <div className="flex flex-col justify-between min-h-[100vh] transition-all duration-500 items-center w-full">
          {/* 메인 중앙 정렬: MOA 타이틀, 소제목, 입력창, 기능 버튼을 하나의 div로 묶음 */}
          <div className="flex flex-col items-center justify-center min-h-[80vh] w-full">
            <h1 className={`text-5xl md:text-6xl font-extrabold mb-4 text-center ${titleColor}`}>MOA</h1>
            <p className={`text-lg md:text-xl mb-10 text-center ${subtitleClasses}`}>당신에게 맞는 AI를 찾아보세요</p>
