...
=======
    try {
      ...
      console.log("🔍 구독 정보 API 응답:", { status: subscriptionResponse.status, ok: subscriptionResponse.ok });

      if (subscriptionResponse.ok) {
        const subscriptionData = await subscriptionResponse.json();
        console.log("🔍 구독 정보 데이터:", subscriptionData);
        setSubscriptionInfo(subscriptionData.activeSubscription ?? null);
      } else {
        console.error("🔍 구독 정보 API 실패:", subscriptionResponse.status);
      }
    } catch (error) {
