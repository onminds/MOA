...
export default function PaymentButton({ planId, planName, amount, currentPlan, onSuccess, onError }: PaymentButtonProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 2. 토스페이먼츠 자동결제(빌링) 카드 등록창 호출
      console.log('토스페이먼츠 자동결제 카드 등록창 호출...');
      
      // 공식 가이드에 맞는 requestBillingAuth 사용
      // successUrl과 failUrl을 통해 사용자가 카드 등록 완료 후 해당 페이지로 이동
      // 빌링키 발급은 success 페이지에서 처리
      try {
        ...
        console.log('토스페이먼츠 카드 등록창 응답:', response);
        console.log('카드 등록창이 열렸습니다. 카드 정보 입력을 완료해주세요.');
        console.log('카드 등록 완료 후 success 페이지에서 빌링키 발급이 진행됩니다.');
      } catch (billingError) {
